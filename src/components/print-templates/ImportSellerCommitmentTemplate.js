...
import { ImportItemFields, ImportOrderFields, formatDate } from '../../models';

const ImportSellerCommitmentTemplate = React.forwardRef(({ importOrder }, ref) => {
  ...
  const sellerAddress = importOrder[ImportOrderFields.SELLER_ADDRESS] || 'N/A';
  const sellerIdNumber = importOrder[ImportOrderFields.SELLER_ID_NUMBER] || 'N/A';
  const sellerPhone = importOrder[ImportOrderFields.SELLER_PHONE] || 'N/A';
  const sellerIssuedDate = importOrder[ImportOrderFields.SELLER_ID_ISSUED_DATE]
  ...
  return (
    <div ref={ref} className="print-seller-commitment-container">
      ...
      <div className="seller-commitment-title">Biên bản cam kết bán tài sản</div>
      ...
      <div className="seller-commitment-intro">Tôi xác nhận và cam kết như sau:</div>
      <div className="seller-commitment-section-title">1. Hàng hóa bán cho Bên mua có thông tin:</div>

      <div className="seller-commitment-items">
        {items.map((item, index) => (
          <div
            key={item[ImportItemFields.ID] || `${item[ImportItemFields.PRODUCT_ID] || 'item'}-${index}`}
          ...
          >
            ...
            <div className="seller-commitment-item-row">
              ...
              <span className="value">{item[ImportItemFields.QUANTITY] || 0}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="seller-commitment-list">
        <div className="seller-commitment-paragraph">
          ...
          <span className="text">
            Tài sản nêu trên là tài sản thuộc quyền sở hữu hợp pháp của tôi, do tôi tự nguyện bán lại cho Bên mua.
          </span>
        </div>
        <div className="seller-commitment-paragraph">
          <span className="index">3.</span>
          <span className="text">
            Tài sản không phải tài sản trộm cắp, không có nguồn gốc bất hợp pháp, không thuộc diện tranh chấp, cầm cố, thế chấp, không bị ràng buộc bởi bất kỳ nghĩa vụ pháp lý nào.
          </span>
        </div>
        <div className="seller-commitment-paragraph">
          <span className="index">4.</span>
          <span className="text">
            Tôi chịu hoàn toàn trách nhiệm trước pháp luật nếu có bất kỳ khiếu nại, tranh chấp, hoặc xác minh từ cơ quan chức năng liên quan đến nguồn gốc tài sản.
          </span>
        </div>
        <div className="seller-commitment-paragraph">
          <span className="index">5.</span>
          <span className="text">
            Sau khi bàn giao tài sản và nhận đủ tiền, tôi không có bất kỳ khiếu nại, yêu cầu hay tranh chấp nào đối với Bên mua.
          </span>
        </div>
        <div className="seller-commitment-paragraph">
          <span className="index">6.</span>
          <span className="text">
            Bên mua không chịu trách nhiệm đối với các vấn đề phát sinh liên quan đến nguồn gốc tài sản trước thời điểm giao dịch.
          </span>
        </div>
      </div>
